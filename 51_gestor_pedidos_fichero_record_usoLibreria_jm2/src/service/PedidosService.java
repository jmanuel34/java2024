...
//		DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd");
		try(FileReader fr=new FileReader(fichero);
			...
			while((linea=bf.readLine())!=null) {
				...
				if (fMin.isBefore(pedido.fechaPedido())) {
					fMin = pedido.fechaPedido();
			}
		}
		}catch(IOException ex) {
			ex.printStackTrace();
...
//		DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd");
		...
		try(FileReader fr=new FileReader(fichero);
			...
			while((linea=bf.readLine())!=null) {
				 pedido = Util.convertirCadenaAPedido(linea);
				if (pedido.fechaPedido().isAfter(f1)&&
						pedido.fechaPedido().isBefore(f2)) {
					pedidos.add(pedido);
			}
		}
		}catch(IOException ex) {
	...
	public Pedido pedidoProximoFecha(LocalDate fecha) {
		...
		try(FileReader fr=new FileReader(fichero);
			...
			while((linea=bf.readLine())!=null) {
				 pedido = Util.convertirCadenaAPedido(linea);
					if(Math.abs(ChronoUnit.DAYS.between(pedido.fechaPedido(), fecha))<
